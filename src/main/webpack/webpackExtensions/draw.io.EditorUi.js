...
(function () {
            ...
            if (this.doImportVisio) {
                ...
                if (remote) {
                    if (VSD_CONVERT_URL != null && !this.isOffline()) {
                        ...
                        onerror({
                            message: this.getServiceName() == 'conf' ? mxResources.get('vsdNoConfig') : mxResources.get('serviceUnavailableOrBlocked')
                        });
                    }
                } else {
                    try {
    ...
    EditorUi.prototype.insertTextAt = function (text, dx, dy, html, asImage, crop, resizeImages, addNewPage) {
        ...
        if (text != null) {
            if (Graph.fileSupport && !this.isOffline() && new XMLHttpRequest().upload && this.isRemoteFileFormat(text)) {
                ...
                this.parseFile(new Blob([text.replace(/\s+/g, ' ')], {
    ...
    EditorUi.prototype.importFile = function (data, mimeType, dx, dy, w, h, filename,
        ...
        if (mimeType.substring(0, 5) == 'image') {
            ...
            // Returns empty cells array as it is aysynchronous
            this.parseFile((file != null) ? file : new Blob([data], {
                type: 'application/octet-stream'
            }), mxUtils.bind(this, function (xhr) {
                if (xhr.readyState == 4) {
                    if (xhr.status >= 200 && xhr.status <= 299) {
            ...
            if (value) {
                ...
                this.menus.defaultFonts = [{
                    ...
                    },
                    {
                        ...
                        'fontUrl': 'https://fonts.googleapis.com/css?family=Rock+Salt'
                    },
                    {
